...
class ExponentialBackoffIntervalTests(TestCase):
    def test_backoff_progression_with_various_bases(self):
        factor = 25
        maximum = 52000
        ...
        for base in [2, 3, 4]:
            for retries, expected_delay in zip(retries_range, expected_delays[base]):
                ...
                self.assertEqual(
                    actual_delay,
                    expected_delay,
                    msg=f"Base={base}, Retry={retries}: Expected {expected_delay}, got {actual_delay}",
                )
