...
class Kanaal:
    def __init__(
        ...
        for kenmerk in self.kenmerken:
            try:
                ...
                model_name = main_resource._meta.model_name
    ...
    def get_field(model: type[Model], field_name: str) -> Field:
        ...
        Supports Mocks during testing by avoiding strict isinstance checks
        unless absolutely necessary.
        """
        try:
            # In Django, get_field returns a Field object.
            # In tests, it returns a Mock.
            return model._meta.get_field(field_name)  # type: ignore
        except FieldDoesNotExist:
            raise TypeError(f"{field_name} does not exist on {model}")
