...
class NotificationMixinBase(type):
    def __new__(cls, name, bases, attrs):
        ...
        router = SimpleRouter()
        get_routes = cast(Callable[[type], List[Any]], router.get_routes)
        for route in get_routes(new_cls):
            mapping = cast(Dict[str, str], route.mapping)
            ...
            for method, action in mapping.items():
                ...
                usage_list = cast(List[str], kanaal.usage[resource])
    ...
    def construct_message(
        ...
        if model is kanaal.main_resource:
            ...
            serializer = serializer_class(
                main_object, context={"request": this.request}
            )
            main_object_data = serializer.data

        action_name = action or getattr(this, "action", "unknown")
        ...
        message_data: Dict[str, Any] = {
            ...
            "kenmerken": kanaal.get_kenmerken(
                main_object, main_object_data, request=getattr(this, "request", None)
            ),
        }
...
class NotificationDestroyMixin(NotificationMixin):
    def destroy(self, request, *args, **kwargs):
        ...
        with conditional_atomic(self.notifications_wrap_in_atomic_block)():
            ...
            instance = this.get_object()
            data = this.get_serializer(instance).data
